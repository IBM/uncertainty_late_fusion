...
class UncertaintyLateFusion(LateFusion):
    ...
    def train(self, predictions, ground_truth):
        ...
        return fusion_weights of size [n_mod, n_class].
        ...
        self.weights = np.zeros((self.n_mods, self.n_classes))
        ...
        for class_id in range(self.n_classes):
            ...
            self.weights[selected[class_id, 0 : self.k], class_id] = np.matrix.flatten(
                w
            )
